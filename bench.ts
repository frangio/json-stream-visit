import { visit } from './src/index.ts';
...
const dataPath = './wiki_movie_plots_deduped.json';
...
Deno.bench('jq (non-streaming)', async () => {
  let output = await new Deno.Command('jq', {
    args: ['max_by(.Plot | length).Plot | length', dataPath],
  }).output();
  let maxPlotLength = Number(String.fromCharCode(...output.stdout));
  checkResult(maxPlotLength);
});

Deno.bench('json-stream-parse', { baseline: true }, async () => {
  let maxPlotLength = 0;
  await visit(streamChunks(data), {
    values: {
      entries: {
        Plot(plot: any) {
